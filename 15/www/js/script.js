window.addEventListener('DOMContentLoaded', () => {
	...
	// function getTotal(target, input) {
	// 	target.addEventListener('input', () => {
	// 		console.log(target.value);

	// 		if(target.value && input.value) {
	// 			total = (target.value * input.value) * defaultPrice;
	// 		} else {
	// 			total = 0;
	// 		}
	...
	// 	let sum = total;
	// 	totalValue.textContent = sum * e.target.value;
	// });

	// getTotal(persons, restDays);
	// getTotal(restDays, persons);

	const counterPrice = document.getElementById('price'),
				persons = counterPrice.querySelectorAll('.counter-block-input')[0],
				restDays = counterPrice.querySelectorAll('.counter-block-input')[1],
				place = counterPrice.querySelector('#select'),
				totalValue = counterPrice.querySelector('#total'),
				defaultPrice = 4000;
	...
	function getTotal() {
		if(persons.value && restDays.value) {
			total = persons.value * restDays.value * place.value * defaultPrice;
		} else {
			total = 0;
		}
	...
	counterPrice.addEventListener('input', () => {
		getTotal();
	});
});
